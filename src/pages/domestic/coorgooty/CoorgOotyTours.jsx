...
/* ============================================================================
   COORG · OOTY · WAYANAD HILL-STATION TOURS
   ----------------------------------------------------------------------------
...
/* ---------- Small decorative rolling-hill arch used on card tops ---------- */
...
function TourCard({ tour, onOpen }) {
  ...
  return (
    <button
      onClick={() => onOpen(tour)}
      className="group relative text-left rounded-2xl overflow-hidden bg-[#F6F7EF] border border-[#1F3D2B]/10 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#2E7D4F] focus-visible:ring-offset-2 focus-visible:ring-offset-[#E3ECDD]"
    >
      <div className="relative h-44 w-full overflow-hidden">
        <TourImage image={cover} />
        <ArchAccent className="absolute -bottom-px left-0 w-full h-8 text-[#F6F7EF]" />
        <span className="absolute top-3 right-3 flex items-center gap-1 rounded-full bg-black/35 backdrop-blur px-3 py-1 text-[11px] tracking-wide uppercase text-[#F3F0DD]">
      ...
      <div className="p-5">
        <div className="flex items-center gap-1.5 text-[#2E7D4F] text-[11px] font-semibold tracking-[0.14em] uppercase mb-2">
          <MapPin size={13} strokeWidth={2.5} />
          {tour.region}
        </div>
        <h3
          className="text-[#1F3D2B] text-xl leading-snug mb-1.5"
          style={{ fontFamily: "'Fraunces', serif" }}
        >
          {tour.title}
        </h3>
        <p className="text-[#3E5C4E]/80 text-sm leading-relaxed">{tour.tagline}</p>

        <span className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-[#2E7D4F] group-hover:gap-2 transition-all">
          View itinerary
  ...
  return (
    <div className="relative h-56 sm:h-72 w-full overflow-hidden rounded-t-2xl">
      ...
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 to-transparent pointer-events-none">
        <span
          className="block p-5 text-[#F3F0DD] text-lg sm:text-xl"
          style={{ fontFamily: "'Fraunces', serif" }}
        >
          {current.label}
        </span>
    ...
    return (
      <div className="mb-2 flex justify-center">
        <img
          src={urls[0]}
          alt=""
          className="w-full max-w-sm h-44 object-cover rounded-lg border border-[#1F3D2B]/10"
        />
      </div>
    );
  }
    ...
    <div className="mb-2 grid grid-cols-2 gap-2">
      {urls.map((u, i) => (
        <img
          ...
          src={u}
          alt=""
          className="w-full h-32 sm:h-36 object-cover rounded-lg border border-[#1F3D2B]/10"
        />
      ))}
    </div>
  );
...
function TourModal({ tour, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="absolute inset-0 bg-[#132A1C]/70 backdrop-blur-sm"
        onClick={onClose}
      />

      <div className="relative w-full max-w-2xl max-h-[88vh] overflow-y-auto rounded-2xl bg-[#F6F7EF] shadow-2xl">
        <button
          onClick={onClose}
          aria-label="Close"
        ...
        <div className="p-6 sm:p-8">
          ...
          <h2
            className="text-[#1F5C39] text-2xl sm:text-3xl leading-tight mb-2"
            style={{ fontFamily: "'Fraunces', serif" }}
          >
            {tour.title}
          </h2>
          <p className="text-[#1F3D2B]/70 text-sm sm:text-base mb-8">{tour.tagline}</p>

          {/* misty-trail itinerary timeline */}
            ...
            <div className="space-y-8">
              {tour.itinerary.map((d) => (
                <div key={d.day} className="relative">
                  <span className="absolute -left-9 top-0 grid place-items-center h-7 w-7 rounded-full bg-[#2E7D4F] text-[#F3F0DD] text-xs font-bold ring-4 ring-[#F6F7EF]">
                    {d.day}
                  </span>
                  <h3 className="text-[#1F5C39] font-semibold text-base sm:text-lg mb-1">
                    Day {d.day}: {d.title}
                  </h3>
                  <p className="text-[#1F3D2B]/80 text-sm leading-relaxed mb-2">{d.body}</p>
                        ...
                        return (
                          <li key={i}>
                            ...
                            <div className="flex gap-2">
                              ...
                              <span className="text-[#1F3D2B]/80 text-sm leading-relaxed">
                                {text}
                              </span>
                            </div>
                          </li>
          ...
          <button
            onClick={onClose}
            className="mt-8 w-full sm:w-auto rounded-full bg-[#2E7D4F] hover:bg-[#1F5C39] transition-colors text-[#F3F0DD] text-sm font-semibold px-6 py-3"
          >
...
export default function CoorgOotyTours() {
  const [activeTour, setActiveTour] = useState(null);

  return (
    <div className="min-h-screen bg-[#E3ECDD] px-5 sm:px-10 py-14">
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Fraunces:wght@500;600&family=Mukta:wght@400;500;600;700&display=swap');
        * { font-family: 'Mukta', sans-serif; }
      `}</style>

      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-1.5 text-[#2E7D4F] text-xs font-semibold tracking-[0.2em] uppercase mb-3">
            <Sparkles size={14} />
            Coorg · Ooty · Wayanad Tours
          </div>
          <h1
            className="text-[#1F5C39] text-4xl sm:text-5xl leading-tight"
            style={{ fontFamily: "'Fraunces', serif" }}
          >
            Misty hills of the Western Ghats
          </h1>
          <p className="text-[#1F3D2B]/60 mt-3 max-w-xl mx-auto text-sm sm:text-base">
