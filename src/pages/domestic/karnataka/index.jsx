...
/* ============================================================================
   KARNATAKA TOURS (Hampi \u00b7 Gokarna \u00b7 Honnavar \u00b7 Murdeshwar)
   ----------------------------------------------------------------------------
...
/* ---------- Small decorative boulder-hill arch used on card tops ---------- */
...
function TourCard({ tour, onOpen }) {
  ...
  return (
    <button
      onClick={() => onOpen(tour)}
      className="group relative text-left rounded-2xl overflow-hidden bg-[#F7F2E7] border border-[#33241A]/10 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#B5651D] focus-visible:ring-offset-2 focus-visible:ring-offset-[#E6E9DE]"
    >
      <div className="relative h-44 w-full overflow-hidden">
        <TourImage image={cover} />
        <ArchAccent className="absolute -bottom-px left-0 w-full h-8 text-[#F7F2E7]" />
        <span className="absolute top-3 right-3 flex items-center gap-1 rounded-full bg-black/35 backdrop-blur px-3 py-1 text-[11px] tracking-wide uppercase text-[#F2E9D2]">
      ...
      <div className="p-5">
        <div className="flex items-center gap-1.5 text-[#B5651D] text-[11px] font-semibold tracking-[0.14em] uppercase mb-2">
          <MapPin size={13} strokeWidth={2.5} />
          {tour.region}
        </div>
        <h3
          className="text-[#33241A] text-xl leading-snug mb-1.5"
          style={{ fontFamily: "'Marcellus', serif" }}
        >
          {tour.title}
        </h3>
        <p className="text-[#0F6357]/80 text-sm leading-relaxed">{tour.tagline}</p>

        <span className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-[#0F6357] group-hover:gap-2 transition-all">
          View itinerary
  ...
  return (
    <div className="relative h-56 sm:h-72 w-full overflow-hidden rounded-t-2xl">
      ...
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 to-transparent pointer-events-none">
        <span
          className="block p-5 text-[#F2E9D2] text-lg sm:text-xl"
          style={{ fontFamily: "'Marcellus', serif" }}
        >
          {current.label}
        </span>
    ...
    return (
      <div className="mb-2 flex justify-center">
        <img
          src={urls[0]}
          alt=""
          className="w-full max-w-sm h-44 object-cover rounded-lg border border-[#33241A]/10"
        />
      </div>
    );
  }
    ...
    <div className="mb-2 grid grid-cols-2 gap-2">
      {urls.map((u, i) => (
        <img
          ...
          src={u}
          alt=""
          className="w-full h-32 sm:h-36 object-cover rounded-lg border border-[#33241A]/10"
        />
      ))}
    </div>
  );
...
function TourModal({ tour, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="absolute inset-0 bg-[#1E1712]/70 backdrop-blur-sm"
        onClick={onClose}
      />

      <div className="relative w-full max-w-2xl max-h-[88vh] overflow-y-auto rounded-2xl bg-[#F7F2E7] shadow-2xl">
        <button
          onClick={onClose}
          aria-label="Close"
        ...
        <div className="p-6 sm:p-8">
          <div className="flex items-center gap-1.5 text-[#0F6357] text-[11px] font-semibold tracking-[0.14em] uppercase mb-2">
            <MapPin size={13} strokeWidth={2.5} />
            {tour.region} · {tour.duration}
          </div>
          <h2
            className="text-[#9A4E10] text-2xl sm:text-3xl leading-tight mb-2"
            style={{ fontFamily: "'Marcellus', serif" }}
          >
            {tour.title}
          </h2>
          <p className="text-[#33241A]/70 text-sm sm:text-base mb-8">{tour.tagline}</p>

          {/* boulder-and-backwater itinerary timeline */}
            ...
            <div className="space-y-8">
              {tour.itinerary.map((d) => (
                <div key={d.day} className="relative">
                  <span className="absolute -left-9 top-0 grid place-items-center h-7 w-7 rounded-full bg-[#B5651D] text-[#F2E9D2] text-xs font-bold ring-4 ring-[#F7F2E7]">
                    {d.day}
                  </span>
                  <h3 className="text-[#0F6357] font-semibold text-base sm:text-lg mb-1">
                    Day {d.day}: {d.title}
                  </h3>
                  <p className="text-[#0F6357]/80 text-sm leading-relaxed mb-2">{d.body}</p>
                        ...
                        return (
                          <li key={i}>
                            ...
                            <div className="flex gap-2">
                              ...
                              <span className="text-[#0F6357]/80 text-sm leading-relaxed">
                                {text}
                              </span>
                            </div>
                          </li>
          ...
          <button
            onClick={onClose}
            className="mt-8 w-full sm:w-auto rounded-full bg-[#B5651D] hover:bg-[#9A4E10] transition-colors text-[#F2E9D2] text-sm font-semibold px-6 py-3"
          >
...
export default function KarnatakaTours() {
  const [activeTour, setActiveTour] = useState(null);

  return (
    <div className="min-h-screen bg-[#E6E9DE] px-5 sm:px-10 py-14">
      <style>{`
      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-1.5 text-[#B5651D] text-xs font-semibold tracking-[0.2em] uppercase mb-3">
            <Sparkles size={14} />
            Karnataka Tours
          </div>
          <h1
            className="text-[#9A4E10] text-4xl sm:text-5xl leading-tight"
            style={{ fontFamily: "'Marcellus', serif" }}
          >
            Boulders, temples and the Konkan coast
          </h1>
          <p className="text-[#33241A]/60 mt-3 max-w-xl mx-auto text-sm sm:text-base">
